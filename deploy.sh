...
update_download_config() {
    ...
    # Build URLs
    local dmg_url="https://github.com/$GITHUB_OWNER/$GITHUB_REPO/releases/download/$tag/$RELEASE_DMG_NAME"
    local release_notes_url="https://github.com/$GITHUB_OWNER/$GITHUB_REPO/releases/tag/$tag"
    local all_releases_url="https://github.com/$GITHUB_OWNER/$GITHUB_REPO/releases"
    local min_macos="${MIN_MACOS:-macOS 12+}"

    # Read existing Windows config if present
    local win_file_size=""
    local win_min_version="Windows 10+"
    local win_sha256=""
    local win_msi_url=""
    local win_exe_url=""
    ...
    if [[ -f "$config_file" ]]; then
        ...
        win_file_size=$(grep -oP "windows:.*?fileSize: '\K[^']*" "$config_file" 2>/dev/null || echo "")
        win_sha256=$(grep -oP "windows:.*?sha256: '\K[^']*" "$config_file" 2>/dev/null || echo "")
        win_msi_url=$(grep -oP "msiUrl: '\K[^']*" "$config_file" 2>/dev/null || echo "")
        win_exe_url=$(grep -oP "exeUrl: '\K[^']*" "$config_file" 2>/dev/null || echo "")
